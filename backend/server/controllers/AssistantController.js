...
`;
      ...
      try {
         ...
         return res.status(500).json({ error: "Error parsing resume details", raw: result });
      }
      return res.status(200).json(resumeJson);
   } catch (error) {
      console.error("extractResumeDetails error:", error);
      return res.status(500).json({ error: "Error extracting resume details", details: error.message });
   }
};
