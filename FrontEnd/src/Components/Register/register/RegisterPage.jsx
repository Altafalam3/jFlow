...
const RegisterPage = () => {
  ...
  const [regCreds, setRegCreds] = useState({
    ...
    phone: "",
    role: "user",
    resume: null, // will hold the actual file
  });
  ...
  const handleFileChange = (e) => {
    setRegCreds({ ...regCreds, resume: e.target.files[0] });
  };
  ...
  const handleRegFormSubmit = async (e) => {
    ...
    try {
      ...
      formData.append("first_name", regCreds.first_name);
      formData.append("last_name", regCreds.last_name);
      formData.append("email", regCreds.email);
      formData.append("password", regCreds.password);
      formData.append("phone", regCreds.phone);
      formData.append("role", regCreds.role);

      // Only append resume if a file was selected
      if (regCreds.resume) {
        formData.append("resume", regCreds.resume);
      }
